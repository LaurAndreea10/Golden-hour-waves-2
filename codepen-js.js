/* GOLDEN HOUR WAVES — interactive ocean with a full day/night cycle
   move the mouse to stir the wind · click / tap for a splash
   #codepenchallenge #cpc-waves */

const cv = document.getElementById('c');
const ctx = cv.getContext('2d');
const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;

const N = 5;            // wave layers
const CYCLE = 60;       // seconds for a full day
const U0 = 0.16;        // start just before golden hour
const TAU = Math.PI * 2;

let W, H, DPR, horizon;
const layers = [];
let stars = [], clouds = [], birds = [];
const foam = [], impulses = [];
let wind = 0, windTarget = 0, drift = 0, ampF = 1;
let swellT = 4;
...
function palette(u){
  const p = u * 4, i = Math.floor(p) % 4, f = smooth(p - Math.floor(p));
  const A = KF[i], B = KF[(i + 1) % 4], o = {};
  for (const k in A) o[k] = mix(A[k], B[k], f);
  return o;
}
...
function resize(){
  ...
  for (let i = 0; i < N; i++){
    const d = i / (N - 1);
    layers.push({
      base: horizon + gap * (i + 0.55), d,
      a1: 5 + d * 15,  f1: 0.0045 + d * 0.002, s1: 0.55 + d * 0.5,
      a2: 3 + d * 9,   f2: 0.011 - d * 0.003,  s2: 0.9 + d * 0.7,
      a3: 1.5 + d * 4, f3: 0.021,              s3: 1.6,
      ph: i * 1.7
    });
  }
  stars = Array.from({ length: 150 }, () => ({
    x: Math.random() * W, y: Math.random() * horizon * 0.96,
    r: Math.random() * 1.3 + 0.3, p: Math.random() * TAU
  }));
  clouds = Array.from({ length: 5 }, () => ({
    x: Math.random() * W, y: horizon * (0.12 + Math.random() * 0.45),
    s: 40 + Math.random() * 70, v: 4 + Math.random() * 7
  }));
  birds = Array.from({ length: 3 }, () => ({
    x: Math.random() * W, y: horizon * (0.25 + Math.random() * 0.3),
    v: 18 + Math.random() * 14, p: Math.random() * TAU
  }));
  if (reduced) draw(5.4, 0);
}

/* ---------- wave height ---------- */
function waveY(L, x, t){
  const l = layers[L];
  ...
  for (let i = 0; i < impulses.length; i++){
    const im = impulses[i], age = t - im.t0, d = x - im.x;
    y += im.amp * (0.3 + l.d * 0.7) * Math.exp(-age * 1.1)
...
function draw(t, dt){
  ...
  const pal = palette(u);
  const alt = Math.cos(u * TAU);          // sun altitude: 1 noon, -1 midnight
  const day = Math.max(0, alt);
  const nf = Math.min(1, Math.max(0, -alt) * 1.6);  // night factor
  ampF = 1 + Math.abs(wind) * 0.85;

  /* sky */
  const sky = ctx.createLinearGradient(0, 0, 0, horizon * 1.15);
  sky.addColorStop(0, css(pal.top));
  sky.addColorStop(1, css(pal.hor));
  ctx.fillStyle = sky;
  ...
  if (nf > 0.02){
    ...
    for (const s of stars){
      ctx.globalAlpha = nf * (0.35 + 0.65 * Math.abs(Math.sin(t * 1.6 + s.p)));
  ...
  if (alt > -0.06){
    ...
    g.addColorStop(0, css(pal.glow, 0.55));
  ...
  if (nf > 0.05){
    ...
    ctx.beginPath(); ctx.arc(moonX, moonY, 18, 0, TAU); ctx.fill();
    ctx.fillStyle = css(pal.top, 0.9);
    ctx.beginPath(); ctx.arc(moonX - 7, moonY - 4, 15, 0, TAU); ctx.fill();
    ctx.globalAlpha = 1;
  }
  ...
  for (const c of clouds){
    ...
    ctx.fillStyle = css(cCol, cloudA);
    for (let k = 0; k < 4; k++){
      ctx.beginPath();
      ctx.ellipse(c.x + (k - 1.5) * c.s * 0.5, c.y + (k % 2) * c.s * 0.1,
                  c.s * (0.65 - Math.abs(k - 1.5) * 0.14), c.s * 0.26, 0, 0, TAU);
      ctx.fill();
    }
  }
  ...
  if (day > 0.12){
    ...
    for (const b of birds){
      b.x += b.v * dt;
      if (b.x > W + 30) { b.x = -30; b.y = horizon * (0.2 + Math.random() * 0.35); }
  ...
  /* ambient swell so it never looks static */
  swellT -= dt;
  if (swellT < 0){
  ...
  for (let i = 0; i < N; i++){
    ...
    /* crest line — turns bioluminescent at night */
    let crest = mix(col, [255, 255, 255], 0.3);
    if (nf > 0.05 && i >= N - 2) crest = mix(crest, [64, 255, 214], nf * 0.9);
    ctx.beginPath();
    ctx.moveTo(0, waveY(i, 0, t));
    for (let x = 4; x <= W + 4; x += 4) ctx.lineTo(x, waveY(i, x, t));
    ctx.strokeStyle = css(crest, 0.35 + (i === N - 1 ? nf * 0.5 : 0));
    ctx.lineWidth = i === N - 1 ? 2 : 1.2;
    ctx.stroke();

    /* surfer rides the middle wave (a nod to 🌊🏄🌊) */
    if (i === 2 && alt > -0.02){
      const sx = ((t * 55) % (W + 160)) - 80;
      const yA = waveY(2, sx - 8, t), yB = waveY(2, sx + 8, t);
      ctx.save();
  ...
  const gl = alt > 0.02 ? { x: sunX, c: pal.sun, a: alt } :
             nf > 0.2   ? { x: moonX, c: [170, 200, 255], a: nf * 0.55 } : null;
  if (gl){
    ctx.save();
  ...
  while (tries--){
    ...
    if (y < layers[front].base - 9 * ampF){
      foam.push({
        x, y, vx: (Math.random() - 0.5) * 30, vy: -10 - Math.random() * 30,
        life: 1, r: 0.8 + Math.random() * 1.8, glow: nf > 0.25 && Math.random() < nf
      });
    }
  }
...
addEventListener('pointermove', e => {
  windTarget = (e.clientX / W - 0.5) * 2;
});

addEventListener('pointerdown', e => {
  const t = performance.now() / 1000;
  impulses.push({ x: e.clientX, t0: t, amp: 30 });
  const y = waveY(N - 1, e.clientX, t);
  for (let i = 0; i < 16; i++){
    foam.push({
      x: e.clientX, y,
      vx: (Math.random() - 0.5) * 240, vy: -60 - Math.random() * 200,
      life: 1, r: 1 + Math.random() * 2.5, glow: false
    });
  }
});
